...
function initializeEpicsWithIds(epics: RawEpic[]): Epic[] {
	return epics.map((epic) => ({
		...epic,
		status: epic.status as 'open' | 'in progress' | 'closed',
		features: epic.features.map((feature: RawFeature) => ({
			...feature,
			status: feature.status as 'todo' | 'ready' | 'in-progress',
			id: feature.id || crypto.randomUUID()
		}))
	}));
}

export const epicsStore = writable<Epic[]>(initializeEpicsWithIds(exampleMapping as RawEpic[]));
...
export function addScenarioToFeature(
	epicId: string,
	featureIndex: number,
...
export function addScenarioToFeatureById(
	epicId: string,
	featureId: string,
	scenarioTitle: string,
	scenarioType: 'green' | 'grey' | 'yellow' = 'grey'
...
export function handleAutomaticStatusTransition(
	epicId: string,
	featureId: string,
	scenarios: Scenario[]
) {
