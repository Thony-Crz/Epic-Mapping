...
- [x] T021 [P] [US3] tiny Extend `frontEnd/accessibility/comprehensive-accessibility.spec.ts` to verify keyboard focus, aria-label, and live-region announcements for the export control.
...
- [x] T024 [US3] small Update epic detail page `frontEnd/src/routes/epics/[id]/+page.svelte` to render the export button, gating on status === Ready, and triggering download via hidden anchor.
- [x] T025 [US3] small Add success/error toast components in `frontEnd/src/ui/toasts/export-ready-epic-toast.svelte` (or existing toast registry) with localization strings.
